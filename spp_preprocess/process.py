...
def preprocess(data: dict, res_name: str) -> dict:
    # Load schema
    with open(PARSER_MAPPING) as fp:
        schemas: dict = json.load(fp)

        schemas = {k: set(v) for k, v in schemas.items()}

    # Start parsing
    out: dict = parser_json(data, res_name, schemas, default_key='member')
